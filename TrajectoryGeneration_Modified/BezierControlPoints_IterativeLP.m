function [Points_Array,T0,T,tau] = BezierControlPoints_IterativeLP(epsilon,alpha_T,X_Array,R_Array,N_pts,T0,vm,am,L_v,L_f,m)
...
for j=1:N_seg   
    ...
    for k=1:N_pts-2
        V((k-1)*3+1:k*3)=((tau(j)^2)/((N_pts-1)*(N_pts-2)))*(min([am+g*e3,[inf;inf;g-(L_f-epsilon)/m]],[],2));   
    end
